...
def battle_loop(player, rival):
    ...
    while battle.battle_ended() == False:
        ...
        player_action_selection = battle.player_choice()
        trainer_action = battle.trainer_action()

        # action of -1 signifies a switch
        if player_action_selection == -1:
            ...
            match(turn_order):
                case 1:
                    ...
                    if battle.trainer_pokemon_fainted() and battle.trainer_two.team_wipe():
                        return "WIN!"
                    battle.trainer_two.team[0].use_move(trainer_action, battle.trainer_two.team[0], battle.trainer_one.team[0])
                    if battle.player_pokemon_fainted() and battle.trainer_one.team_wipe():
                        return "LOST!"
                case 2:
                    battle.trainer_two.team[0].use_move(trainer_action, battle.trainer_two.team[0], battle.trainer_one.team[0])
                    if battle.player_pokemon_fainted() and battle.trainer_one.team_wipe():
